...
class UniformQuantizer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.inited is False:
            if self.minmax:
                self.delta = (x.max() - x.min()) / (self.n_levels - 1)
                self.zero_point = (- x.min() / self.delta).round()
            else:
...
class SymmetricQuantizer(nn.Module):
    """
    PyTorch Function that can be used for symmetric quantization. Quantizes its argument in the forward pass,
    passes the gradient 'straight through' on the backward pass, ignoring the quantization that occurred.
    :param n_bits: number of bit for quantization
    :param channel_wise: if True, compute scale in each channel
    """
    def __init__(self, n_bits: int = 8, channel_wise: bool = False):
    ...
    def __repr__(self):
        s = super(SymmetricQuantizer, self).__repr__()
        s = "(" + s + " inited={}, channel_wise={})".format(self.inited, self.channel_wise)
        return s
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.inited is False:
            if self.minmax:
                self.delta = x.abs().max() / (self.n_levels // 2 - 1)
            else:
    ...
    def init_quantization_scale(self, x: torch.Tensor, channel_wise: bool = False):
        ...
        if channel_wise:
            ...
            delta = x_max.clone()
            # determine the scale channel-by-channel
            for c in range(n_channels):
                if len(x.shape) == 3:
